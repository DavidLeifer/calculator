...
int charBinary2Int(char binaryAdd[], int intBinaryGreatOne) {
  ...
  int lengthPower = power(10, intBinaryGreatOne);
...
int binary2Decimal(int binary, int intBinaryGreatOne, int binaryAddOne) {
  ...
  while (i < intBinaryGreatOne) {
    decimal = decimal + ((charBinaryAdd[i] - '0') * (power(2, j)));
...
void textGUI() {
  ...
  printf("%s\n\n", version);
  printf("Enter two numbers less than 200\nwith arithmatic operands\n +  ,  -  ,  *  ,  /  ,  **\n\nExample: 1 + 1 (press enter):\n\n");
  scanf(" %d%2s%d", &numberOne, operand, &numberTwo);
  char additionChar;
  additionChar = '+';
  ...
  divisionChar = '/';
  char *powerChar = "**";
  // Checks if the input matches operand ('+', '-', '*', 'etc') to run the function.
  if (operand[0] == '*' && operand[1] == '*') {
    int intPower = power(numberOne, numberTwo);
    printf("\n%d ** %d = %d\n", numberOne, numberTwo, intPower);
  }
  else if (operand[0] == additionChar) {
    int sum = addition(numberOne, numberTwo);
    printf("\n%d + %d = %d\n", numberOne, numberTwo, sum);
  }
  else if (operand[0] == subtractionChar) {
    int difference = subtraction(numberOne, numberTwo);
  }
  else if (operand[0] == multiplicationChar) {
    int product = multiplication(numberOne, numberTwo);
  }
  else if (operand[0] == divisionChar) {
